...
class InterviewConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data=None, bytes_data=None):
        if bytes_data is not None:
            if self.authenticated:
                if len(self.audio_buffer) + len(bytes_data) > RUNTIME["interview"]["max_audio_bytes"]:
                    self.audio_buffer = bytearray()
    ...
    def get_interview(self, interview_id):
        return InterviewSession.objects.select_related("job").filter(id=interview_id).first()
    ...
    async def disconnect(self, close_code):
        ...
        if self.finished and self.timeout_task and self.timeout_task is not current_task and not self.timeout_task.done():
