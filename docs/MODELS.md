...
| --- | --- | --- | --- |
| Interviewer + job metadata + final evaluator | `Qwen/Qwen3.6-27B` | GPU 0 | BitsAndBytes NF4 4-bit weights, BF16 compute |
...
Qwen3.6 keeps `attn_implementation='sdpa'` for its 16 full-attention layers. Its 48 Gated DeltaNet layers use the optimized Flash Linear Attention (`fla`) delta-rule kernels and `causal-conv1d` kernels when both packages are available; startup prints the selected DeltaNet backend so fallback execution is visible. The external Dao-AILab `flash-attn` package is still not used.

## Evaluation

Evaluation no longer unloads the realtime stack or launches a second inference engine. The resident Qwen3.6 instance evaluates criteria in microbatches of four, performs the final reasoning pass, then uses the same constrained decoder used elsewhere in the project to return exactly `PROGRESS` or `NOT_PROGRESS`.
